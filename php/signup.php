        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) { // if email is valid
            ...
            if (mysqli_num_rows($sql) > 0) { // if email already exist
                ...
                if (isset($_FILES['image'])) { // if file is uploaded
                    ...
                    $tmp_name = $_FILES['image']['tmp_name']; // this temporary name is used to save/move file in ou folder
                    ...
                    if (in_array($img_ext, $extensions) === true) { // if user uploaded img ext is matched with any array extensions
                        ...
                        // let's moe the user uploaded img to our particular folder
                        $new_img_name = $time.$img_name;
                        if(move_uploaded_file($tmp_name, "images/".$new_img_name)){ // ifuser upload img move to our folder successfully
